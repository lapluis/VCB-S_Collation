...
| 颜色                                                     | 信息                   |
| ------------------------------------------------------- | ----------------------|
| $\Large \color{#92aaf3}{\blacksquare}$ 浅蓝色 | 符合命名规则或后缀的文件  |
| $\Large \color{#fb9966}{\blacksquare}$ 浅橙色 | 不符合命名规则或后缀的文件 |
| $\Large \color{#ff6538}{\blacksquare}$ 深橙色 | CUE 与文件不匹配         |
| $\Large \color{#51559b}{\blacksquare}$ 蓝紫色 | 非 UTF-8 编码的 CUE     |
| $\Large \color{#ff0a32}{\blacksquare}$ 洋红色 | 过长的路径名             |
| $\Large \color{#cfd8dc}{\blacksquare}$ 灰白色 | 无压缩的 FLAC           |
| $\Large \color{#fbbc05}{\blacksquare}$ 明黄色 | 无 BOM 的 UTF-8        |
| $\Large \color{#009933}{\blacksquare}$ 深绿色 | 后缀名与文件头不符       |
| $\Large \color{#0559ae}{\blacksquare}$ 钴蓝色 | CD 文件夹命名与规则不符   |
| $\Large \color{#8b4513}{\blacksquare}$ 深棕色 | LOG 与签名不符          |
| $\Large \color{#2e373b}{\blacksquare}$ 青灰色 | 存在合并字符            |
| $\Large \color{#cad7ce}{\blacksquare}$ 淡绿色 | 文件为空                |
| $\Large \color{#ba55d3}{\blacksquare}$ 浅紫色 | 目录命名有误            |

### 颜色详解及对应处理方法: 错误信息

这一部分介绍 ATI 中会出现的报错, 这些报错往往意味着整理成品中可能存在命名错误、CUE 格式错误或其他整理错误, 往往需要手动修复.

**1. $\color{#fb9966}{\blacksquare}$ 浅橙色: 不符合命名规则或后缀的文件**

- ATI 会检查视频、音频、图片的命名格式是否符合整理标准. 如符合标准则底色为浅蓝色 (默认不显示, 勾选显示全部后显示), 不符合标准则底色为浅橙色.

**2. $\color{#ff6538}{\blacksquare}$ 深橙色: CUE 与文件不匹配**

- ATI 会检查所有 CD 文件夹下 CUE 文件的 `FILE` 字段并尝试与同文件夹下的音频文件匹配, 若无法匹配到所指向的文件则 CUE 文件底色为深橙色.
- 勾选左下角的 `修复 CUE` 可修复**文件名相同**但**扩展名不同**的 `FILE` 字段.

**3. $\color{#51559b}{\blacksquare}$ 蓝紫色: 非 UTF-8 编码的 CUE 文件 / $\color{#fbbc05}{\blacksquare}$ 明黄色: 无 BOM 的 UTF-8 编码的 CUE 文件**

- ATI 使用 [Ude](https://github.com/errepi/ude) 检测 CUE 文件的编码格式. 如与整理规范 (UTF-8 with BOM) 不符则 CUE 文件底色蓝紫色或明黄色.
- 使用文本编辑器修改编码格式即可.

**4. $\color{#ff0a32}{\blacksquare}$ 洋红色: 过长的路径名**

- Windows 系统中, 默认路径长度限制为 256 个字符. 超过这一限制的路径名 (以文件夹放在磁盘根目录开始计算) 的文件会以洋红色底色显示.
- 使用简写以压缩路径名.

**5. $\color{#cfd8dc}{\blacksquare}$ 灰白色: 无压缩或编码等级过低的 FLAC**
...
**6. $\color{#009933}{\blacksquare}$ 深绿色: 后缀名与文件头不符**
